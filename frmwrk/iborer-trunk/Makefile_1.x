...
LIB_NAME=iBorer
LIB_SUFFIX=framework
VERSION=0.1
FIRMWARE_VERSION=U

TARGET=build/$(LIB_NAME).$(LIB_SUFFIX)
LIB=$(TARGET)/$(LIB_NAME)
...
LD=$(CC) -F$(HEAVENLY_SYSTEM)/System/Library/Frameworks
...
LIB_OBJS=src/IBPluginManager.o src/common/OSUtil.o src/common/Logger.o
$(LIB):	$(LIB_OBJS)
	$(LD) $(LDFLAGS) -o $@ $^
	@echo --------------------------------------------------------------------------------
	@echo ">>>> $(LIB_NAME).framework v$(FULL_VERSION) for iPhone($(FIRMWARE_VERSION)) is built successfully! <<<<"
	@echo
	@echo
